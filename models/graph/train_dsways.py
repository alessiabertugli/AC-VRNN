...
from utils.evaluate_model import check_accuracy_graph_sways, evaluate_graph_sways
import os, sys
import logging
...
def train(epoch, train_loader, optimizer, model, args, writer, beta_vals):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if batch_idx % args.print_every == 0:
            ...
            ade, ade_l, ade_nl = cal_ade(pred_traj_sampled, pred_traj_gt, linear_ped=None, non_linear_ped=None)
            fde, fde_l, fde_nl = cal_fde(pred_traj_sampled, pred_traj_gt, linear_ped=None, non_linear_ped=None)
...
def validate(checkpoint, epoch, model, train_loader, validation_loader, optimizer, writer):
    ...
    metrics_val, val_loss = check_accuracy_graph_sways(args, validation_loader, model, epoch)

    writer.add_scalar('loss_train', val_loss, epoch)

    logger.info('Checking stats on train ...')
    metrics_train, _ = check_accuracy_graph_sways(args, train_loader, model, epoch, limit=True)
...
def main(args):
    ...
    dset, train_loader = data_loader_sways(args, 'training')
    tset, test_loader = data_loader_sways(args, 'test')
    vset, validation_loader = data_loader_sways(args, 'val')
    ...
    for epoch in range(index, args.n_epochs + 1):
        ...
        if epoch % args.save_every == 0:
            ...
            ade, fde = evaluate_graph_sways(args, test_loader, model, args.num_samples, epoch)
            ade_val, fde_val = evaluate_graph_sways(args, validation_loader, model, args.num_samples, epoch)
            ...
            print("**** TEST **** ==> Ade , fde: ", ade, fde)
            print("**** VALIDATION **** ==> Ade , fde: ", ade_val, fde_val)
...
if __name__ == '__main__':
    ...
    training_path = os.path.join(os.path.curdir, '../../datasets/sways_dataset/pt/zara2/train')
    test_path = os.path.join(os.path.curdir, '../../datasets/sways_dataset/pt/zara2/test')
    validation_path = os.path.join(os.path.curdir, '../../datasets/sways_dataset/pt/zara2/val')
    hmap_path = os.path.join(os.path.curdir, '../../dataset_processing/local_hm_5x5_sways')
