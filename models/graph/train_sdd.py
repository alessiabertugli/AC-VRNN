...
from utils.adj_matrix import compute_adjs, compute_adjs_knnsim, compute_adjs_distsim
import time
...
def train(epoch, train_loader, optimizer, model, args, writer, beta_vals):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if batch_idx % args.print_every == 0:
            ...
            l2_loss_abs, l2_loss_rel = cal_l2_losses(pred_traj_gt, pred_traj_gt_rel, pred_traj_sampled_rel, pred_traj_sampled, loss_mask)
...
def validate(checkpoint, epoch, model, train_loader, validation_loader, optimizer, writer):
    ...
    metrics_train, _ = check_accuracy_graph(args, train_loader, model, epoch, limit=True)
...
def test(epoch, test_loader, model, writer, beta_vals):
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_loader):
            (obs_traj, pred_traj_gt, obs_traj_rel, pred_traj_gt_rel, non_linear_ped, loss_mask, seq_start_end, maps, dnames) = batch
...
def main(args):
    ...
    dset, train_loader = data_loader_sdd(args, 'training')
    tset, test_loader = data_loader_sdd(args, 'test')
    vset, validation_loader = data_loader_sdd(args, 'val')
...
if __name__ == '__main__':
    ...
    training_path = os.path.join(os.path.curdir, '../../../trj2020/datasets/sdd_npy/training.npy')
    test_path = os.path.join(os.path.curdir, '../../../trj2020/datasets/sdd_npy/test.npy')
    validation_path = os.path.join(os.path.curdir, '../../../trj2020/datasets/sdd_npy/validation.npy')
    hmap_path = os.path.join(os.path.curdir, '../../../trj2020/dataset_processing/local_hm_5x5_sdd')
    ...
    # Variety loss params
    parser.add_argument('--v_loss', action='store_true', help='use variety loss')
    ...
    parser.add_argument('--weight', default=1, type=float, help='kl heatmap weight, if none variance weight will be used')
    ...
    parser.add_argument('--expname', type=str, default='sdd',
                        help='experiment name, for distinguishing different parameter settings.')
    parser.add_argument('--dname', type=str, default='sdd')
