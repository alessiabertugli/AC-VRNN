...
from utils.metrics import cal_ade, cal_fde, l2_loss, displacement_error, final_displacement_error
...
def main(args):
    ...
    model =VRNN(args.x_dim, args.h_dim, args.z_dim, args.n_layers, writer).to(device)
    ...
    for epoch in range(index, args.n_epochs + 1):
        ...
        if epoch % args.save_every == 0:
            ...
            ade_val, fde_val, m_rate_val, mean_l2_val, best_l2_val, max_l2_val = evaluate_baseline(args, validation_loader, model, args.num_samples)
...
if __name__ == '__main__':
    ...
    training_path = os.path.join(os.path.curdir, '../../datasets/sdd_npy/training.npy')
    test_path = os.path.join(os.path.curdir, '../../datasets/sdd_npy/test.npy')
    validation_path = os.path.join(os.path.curdir, '../../datasets/sdd_npy/validation.npy')
    hmap_path = os.path.join(os.path.curdir, '../../dataset_processing/local_hm_5x5_sdd')

    parser.add_argument('--processed', action='store_false')

    parser.add_argument('--model', default='vrnn', type=str, help='gat or gcn')
